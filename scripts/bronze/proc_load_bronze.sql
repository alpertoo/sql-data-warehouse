...
Purpose: This stored procedure loads data into the 'bronze' schema from external files.
Truncates the bronze tables before loading data.
Uses the 'BULK INSERT' command to load data from files to bronze tables.
...
BEGIN
	...
	SET @batch_start_time = GETDATE();
		...
		SET @start_time = GETDATE();
		PRINT '>>> Deleting Table: bronze.crm_prd_info';
		...
		SET @start_time = GETDATE();
		PRINT '>>> Deleting Table: bronze.crm_sales_details';
		...
		SET @start_time = GETDATE();
		PRINT '>>> Deleting Table: bronze.erp_loc_a101';
		...
		SET @start_time = GETDATE();
		PRINT '>>> Deleting Table: bronze.erp_cust_az12';
		...
		SET @start_time = GETDATE();
		PRINT '>>> Deleting Table: bronze.erp_px_cat_g1v2';
